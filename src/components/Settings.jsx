...
const GUIDE = [
  ...
  },
  {
    ...
    items: [
      ...
      ['Permissions', 'The composer’s permissions pill sets how much the agent can do without asking — Ask each (confirm every command), Auto approve (builds, file writes and other ordinary work run silently; only destructive commands such as rm -rf, sudo or format still ask), or Allow all (never ask). MCP tools and computer control always ask under Auto approve, even when they are harmless — only Allow all skips those prompts too. Flip it to Allow all for an unattended long build.'],
