...
const GUIDE = [
  ...
  },
  {
    ...
    items: [
      ...
      ['Preview \u2014 a window on what the agent makes', 'The panel on the right (top-right icon) has a Preview tab. It shows the newest thing the chat produced that a browser can draw: a web page, an image, a diagram, a Markdown document, a PDF \u2014 or a dev server the agent started, the moment it prints a localhost address. A file is shown again when it changes on disk; a dev server refreshes itself. Chips above the frame let you flip between everything the chat has produced, and the address field takes any URL or file path, so it is also just a viewer.'],
      ['For you \u2014 which chats are waiting on you', 'The same panel has a For you tab with a count. It lists every chat that needs you: a command waiting for approval, a question the agent asked, or a reply that finished while you were looking at another chat. Click a row to go there. \u21e7\u2318I opens the list and jumps to the first one.'],
